...
void stream_cipher_ctx_init(cipher_ctx_t* ctx, int method, int enc)
{
    if (method <= TABLE || method >= STREAM_CIPHER_NUM) {
...
void stream_ctx_release(cipher_ctx_t* cipher_ctx)
{
...
int stream_encrypt(buffer_t* plaintext, cipher_ctx_t* cipher_ctx, size_t capacity)
{
    if (cipher_ctx == NULL)
        return CRYPTO_ERROR;
...
int stream_decrypt(buffer_t* ciphertext, cipher_ctx_t* cipher_ctx, size_t capacity)
{
    if (cipher_ctx == NULL)
        return CRYPTO_ERROR;
...
void stream_ctx_init(cipher_t* cipher, cipher_ctx_t* cipher_ctx, int enc)
{
